-- Migration: duo_feed — p_role_filter text → p_role_filters text[]
-- Permet de filtrer sur plusieurs rôles simultanément (ex. ARRAY['MID','ADC'])

create or replace function public.duo_feed(
  p_user_id      uuid,
  p_role_filters text[]  default null,   -- ex. ARRAY['MID','ADC'] ou null = tous rôles
...
begin
  ...
  scored as (
    select
      ...
      -- Role fit — p_role_filters && main_roles (overlap d'arrays)
      case
