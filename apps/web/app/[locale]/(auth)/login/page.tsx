...
function rankLabel(rankKey: string | null, division?: string | null): string {
  if (!rankKey) return 'UNRANKED'
  const k = rankKey.toLowerCase()
  if (RANK_LABEL[k]) return RANK_LABEL[k]
  return `${k.slice(0, 3).toUpperCase()} ${division ?? ''}`.trim()
}
...
function LgBrandPanel() {
  return (
    <div style={{ position: 'relative', flex: '1 1 0', minWidth: 0, display: 'flex', flexDirection: 'column', padding: '40px 56px', overflow: 'hidden', borderRight: `1px solid ${T.line}` }}>
      ...
      <div style={{ position: 'relative', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', maxWidth: 460 }}>
        ...
        <h1 style={{ margin: 0, fontFamily: T.display, fontSize: 68, lineHeight: 0.94, letterSpacing: '-0.01em', color: T.text }}>
          BON<br />
          <span style={{ background: `linear-gradient(120deg, ${T.cyan}, ${T.violet})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
            RETOUR.
          </span>
        </h1>
...
function LgRecentAccount({ account, onClick }: { account: RecentAccount; onClick: () => void }) {
  const rkColor = RANK_COLORS[account.rankKey ?? 'iron'] ?? T.textMute
  const label = rankLabel(account.rankKey)
  ...
  return (
    <button onClick={onClick} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px', borderRadius: 15, background: `linear-gradient(120deg, ${T.cyan}14, ${T.violet}10)`, border: `1px solid ${T.cyan}55`, boxShadow: `0 0 26px -10px ${T.cyan}66`, cursor: 'pointer', textAlign: 'left' }}>
      ...
      <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 42, height: 42, borderRadius: 12, background: `linear-gradient(135deg, ${T.cyan}, ${T.violet})`, boxShadow: `0 10px 24px -10px ${T.cyan}`, flexShrink: 0 }}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#001018" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round">
...
function LgConnectCard({ account, onRiotLogin }: { account: RecentAccount | null; onRiotLogin: () => void }) {
  const hasAccount = account !== null

  return (
    <div style={{ flex: '0 0 520px', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '40px 64px', position: 'relative' }}>
      ...
      <div style={{ width: '100%', maxWidth: 392, margin: '0 auto' }}>
        ...
        {hasAccount && (
          <>
            ...
            <LgRecentAccount account={account} onClick={onRiotLogin} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 18, marginTop: 18, flexWrap: 'wrap' }}>
          {[
            ...
            <span key={label} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: T.mono, fontSize: 9.5, color: T.textMute, letterSpacing: '0.04em' }}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke={T.live} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d={path} />
              </svg>
              {label}
            </span>
        ...
        <div style={{ marginTop: 30, paddingTop: 22, borderTop: `1px solid ${T.line}`, display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ fontFamily: T.body, fontSize: 14, color: T.textDim }}>
            ...
            <Link href="/onboarding/1" style={{ fontFamily: T.display, fontSize: 13, letterSpacing: '0.06em', color: T.cyan, textDecoration: 'none', textTransform: 'uppercase' }}>
              Créer un compte →
            </Link>
          </div>
...
export default function LoginPage() {
  ...
  function handleRiotLogin() {
    // TODO: déclencher le flow RSO Riot réel (supabase.auth.signInWithOAuth provider: 'riot')
    // En attendant la config RSO : redirige vers l'onboarding (identique au CTA landing)
    window.location.href = `/${window.location.pathname.split('/')[1]}/onboarding/1`
  }

  // Évite le flash de contenu (chip présent → absent) avant la lecture localStorage
  if (!hydrated) return (
  ...
  return (
    <div style={{
      ...
      <LgBrandPanel />
      <LgConnectCard account={account} onRiotLogin={handleRiotLogin} />
    </div>
  )
}
