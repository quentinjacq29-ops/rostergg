...
export default async function MePage({ params: { locale } }: Props) {
  ...
  const [
    ...
    supabase.from('matching_prefs').select('main_roles, looking_for_roles, champion_pool, playstyles, languages, voice_required, duo_search_active').eq('profile_id', user.id).maybeSingle(),
  ...
  return (
    <MeClient
      ...
      duoSearchActive={prefs?.duo_search_active ?? true}
