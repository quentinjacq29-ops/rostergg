...
export default function DuoRequestModal({
  ...
  return (
    // Backdrop — centré desktop, ancré en bas (bottom-sheet) en mobile
    <div className="rgg-dr-backdrop" onClick={e => { if (e.target === e.currentTarget) onClose() }} style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(6,7,11,0.72)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div className="rgg-dr-panel" style={{ width: 540, maxHeight: '90vh', display: 'flex', flexDirection: 'column', borderRadius: 22, overflow: 'hidden', position: 'relative', background: `linear-gradient(180deg, ${T.surface}, ${T.bg})`, border: `1px solid ${T.cyan}44`, boxShadow: `0 44px 100px -34px rgba(0,0,0,0.88), 0 0 70px -24px ${T.cyan}` }}>
        {/* glow */}
        ...
        {/* poignée — mobile only */}
        <div className="rgg-dr-grab" style={{ width: 40, height: 4, borderRadius: 3, background: T.lineStrong, margin: '10px auto 0', flexShrink: 0 }} />

        {/* Header */}
        <div style={{ position: 'relative', flexShrink: 0, padding: '20px 24px 16px', borderBottom: `1px solid ${T.line}` }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontFamily: T.mono, fontSize: 10, color: T.cyan, letterSpacing: '0.2em' }}>
              {sent ? '◢ DEMANDE ENVOYÉE' : '◢ DEMANDE DE DUO'}
            </span>
        ...
        {sent ? (
          /* ── État ENVOYÉE */
          <>
            <div style={{ flex: 1, overflowY: 'auto', position: 'relative', padding: '30px 24px 22px', textAlign: 'center' }}>
              ...
              <h2 style={{ margin: 0, fontFamily: T.display, fontSize: 23, color: T.text, letterSpacing: '0.02em' }}>Demande envoyée à {target.name}</h2>
            ...
            <div className="rgg-dr-actions" style={{ flexShrink: 0, display: 'flex', gap: 11, padding: '14px 24px calc(16px + env(safe-area-inset-bottom))', borderTop: `1px solid ${T.line}`, background: T.bg }}>
              <button onClick={() => setState('compose')} style={{ flex: 1, height: 50, borderRadius: 13, background: 'rgba(255,255,255,0.05)', border: `1px solid ${T.lineStrong}`, color: T.text, cursor: 'pointer', fontFamily: T.display, fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>
                Retour
              </button>
              <button onClick={() => { onClose(); router.push('/fr/inbox') }} style={{ flex: 1.6, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, height: 50, borderRadius: 13, border: 'none', background: `linear-gradient(135deg, ${T.cyan}, ${T.violet})`, color: '#001018', cursor: 'pointer', fontFamily: T.display, fontSize: 13.5, letterSpacing: '0.07em', textTransform: 'uppercase' as const, fontWeight: 700, boxShadow: `0 14px 32px -12px ${T.cyan}` }}>
          ...
          /* ── État COMPOSE */
          <>
            <div style={{ flex: 1, overflowY: 'auto', position: 'relative', padding: '18px 24px 20px' }}>
              ...
              <textarea
                ...
                rows={3}
                style={{ width: '100%', boxSizing: 'border-box', resize: 'none', minHeight: 96, padding: '13px 15px', borderRadius: 13, background: 'rgba(255,255,255,0.035)', border: `1px solid ${T.lineStrong}`, color: T.text, fontFamily: T.body, fontSize: 14, lineHeight: 1.5, outline: 'none' }}
              />

              {/* Suggestions — liste pleine largeur (maquette mobile) */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 12 }}>
                {SUGGESTIONS.map((s, i) => (
                  <button key={i} onClick={() => setMessage(s)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 13px', borderRadius: 11, background: 'rgba(255,255,255,0.04)', border: `1px solid ${T.line}`, cursor: 'pointer', fontFamily: T.body, fontSize: 12.5, color: T.textDim, textAlign: 'left', lineHeight: 1.35 }}>
            ...
            {/* CTAs — barre sticky */}
            <div className="rgg-dr-actions" style={{ flexShrink: 0, display: 'flex', gap: 11, padding: '14px 24px calc(16px + env(safe-area-inset-bottom))', borderTop: `1px solid ${T.line}`, background: T.bg }}>
              <button onClick={onClose} style={{ flex: 1, height: 50, borderRadius: 13, background: 'rgba(255,255,255,0.05)', border: `1px solid ${T.lineStrong}`, color: T.text, cursor: 'pointer', fontFamily: T.display, fontSize: 13, letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>
                Annuler
              </button>
              <button onClick={handleSend} disabled={loading} style={{ flex: 1.6, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8, height: 50, borderRadius: 13, border: 'none', background: loading ? 'rgba(255,255,255,0.08)' : `linear-gradient(135deg, ${T.cyan}, ${T.violet})`, color: loading ? T.textDim : '#001018', cursor: loading ? 'not-allowed' : 'pointer', fontFamily: T.display, fontSize: 13.5, letterSpacing: '0.07em', textTransform: 'uppercase' as const, fontWeight: 700, boxShadow: loading ? 'none' : `0 14px 32px -12px ${T.cyan}`, whiteSpace: 'nowrap' }}>
                {loading ? (
